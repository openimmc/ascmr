...
def gen_fea(inp, k=3, t=0.3):
    fea = inp / torch.norm(inp, dim=1, keepdim=True)
    dist = torch.matmul(fea, fea.T)
    dist[dist < t] = 0
    dist[dist >= t] = 1
    dist = dist * 0.5
    #ord = dist.argsort(descending=True)
    #adj = torch.zeros_like(dist)
    #for row in range(inp.shape[0]):
    #    for i in range(k+1):
    #        adj[row, ord[row, i]] = 0.5
    #        adj[ord[row, i], row] = 0.5
    return dist
